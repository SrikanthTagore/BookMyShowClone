...
export default function PaymentModal({isOpen, setIsOpen, price}) {
  ...
  const launchRazoPay = () => {
      let options = {
        key: 'rzp_test_IxyiroIxFUWUtN',
        amount: price * 100,
        currency: "INR",
        name: "Book My Show Clone",
        description: "Movie Purchase or Rental",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRh1POx70rAnULC-sZC_UbH4LXhy9jsr2MmuJzSmmlMIMD7yCK3",
        handler: () => {
            setIsOpen(false);
              ...
              <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                ...
                <div className=" w-full mt-4">
                  <button
                    ...
                    onClick={launchRazoPay}
                  >
                    pay ₹ {price}
                  </button>
                  <button
                    type="button"
                    className="w-full mt-3 inline-flex justify-center px-4 py-2 text-sm font-medium text-red-700 bg-red-200 border border-transparent rounded-md"
                    onClick={closeModal}
                  >
                    Cancel Payment
                  </button>
